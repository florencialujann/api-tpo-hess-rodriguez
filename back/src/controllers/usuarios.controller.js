...
class UsuariosController {
  ...
  async getUsuarios(req, res) {
    try {
      ...
      return res.status(200).json(users); //el controlador devuelve el codigo del estado 200 cuando devuelve correctamente. La respuesta se devuelve al front.
    } catch (err) {
      console.error(err);
  ...
  async login(req, res) {
    try {
      ...
      if (isUserRegistered) {
        ...
        const token = jwt.sign(user.toJSON(), process.env.PRIVATE_KEY, {//se encripta el usuario con el metodo sign
          expiresIn: "1d",
        });
